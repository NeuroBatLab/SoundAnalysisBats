...
for ff=1:NFiles
    % onset and offset of manually detected vocalization events for all 4
    % loggers
...
for ll=1:NLoggers
    for vv=1:size(ManCallTimes{ll},1)
        ...
        Idx_WBound = find(sum((allCallTimes{Ord(ll)}>OnOffVoc(1)-500) .* (allCallTimes{Ord(ll)}<OnOffVoc(2)+500), 2)>0);
        ...
        if isempty(Idx_WBound)
            ...
            for ii=1:length(Idx_WBound)
                plot(allCallTimes{Ord(ll)}(Idx_WBound(ii),:), ones(2,1), 'b-', 'LineWidth',2)
                hold on
            end
            plot(ManCallTimes{ll}(vv,:), ones(2,1), 'g-', 'LineWidth',1)
            hold off
