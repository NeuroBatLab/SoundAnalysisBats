...
def discriminatePlot(X, y, cVal, titleStr='', figdir='.', Xcolname = None, plotFig = False, removeTickLabels = False, testInd = None, group_hold_out = pd.Series([])):
    ...
    if not group_hold_out.empty:
        #pdb.set_trace()
        group_good = group_hold_out[goodInd]
    ...
    if testInd is None:
        ...
        if not group_hold_out.empty:
            skf = StratifiedGroupKFold(n_splits = cvFolds)
            skfList = skf.split(Xr, yGood, group_good)
            #pdb.set_trace()
            print(f"doing group holdout")
    else:
    ...
    for train, test in skfList:
        ...
        #pdb.set_trace()
        # group_good2 = group_good.to_numpy()
        # num_train = len(set(group_good2[train]))
        # num_test = len(set(group_good2[test]))
        # testbats = set(group_good2[test])
        # print( f"Fold# {cvFolds} num_train: {num_train} num_test: {num_test} testbats {testbats}" )
...
#    withinCov = np.average(covs, axis=0, weights=myPrior)
    ...
    weights_all = np.dot(ldaMod.scalings_[:,0:3].T, pca.components_)
    ...
    return ldaYes, qdaYes, rfYes, cvCount, ldaP, qdaP, rfP, nClasses, weights, weights_all
