...
if isempty(DataFiles)
    ...
    for df=1:length(DataFiles)
        ...
        for vv=vv:Nvoc
            ...
            if sum(cellfun('isempty',(Amp_env_LowPassLogVoc{vv}))) == length(AudioLogs) % No logger data, just isolate onset/offset of vocalizations on the microphone
                if ~Force_Save_onoffsets_mic
                    ...
                    if CheckMicChannel
                        ...
                        hold on
                        plot([0 length(Amp_env_Mic{vv})], (Factor_RMS_Mic * MeanStdAmpRawExtract(Nvocs(df)+vv,1))*ones(2,1).*10^3, 'Color',ColorCode(ll+1,:),'LineStyle','--');
                        hold on
                    end
                    ...
                    VocpMic = Amp_env_Mic{vv}>(Factor_RMS_Mic * MeanStdAmpRawExtract(Nvocs(df)+vv,1)); % Time points above amplitude threshold on the band-pass microphone signal
